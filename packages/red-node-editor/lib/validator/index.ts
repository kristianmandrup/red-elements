...
export class NodeValidator extends Context implements INodeValidator {
  ...
  validateNode(node) {
    ...
    if (node.type.indexOf("subflow:") === 0) {
      subflow = $nodes.subflow(node.type.substring(8));
      ...
      var subflowNodes = $nodes.filterNodes({
        z: node.id
      });
      ...
      var subflowInstances = $nodes.filterNodes({
  ...
  validateNodeProperty(node: INode, definition: any, property: string, value: string) {
    ...
    if (valid && definition[property].type && $nodes.getType(definition[property].type) && !("validate" in definition[property])) {
      if (!value || value == "_ADD_") {
        valid = definition[property].hasOwnProperty("required") && !definition[property].required;
      } else {
        var configNode = $nodes.node(value);
