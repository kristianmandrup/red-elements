...
export class LabelBuilder extends Context implements ILabelBuilder {
  ...
  buildLabelRow(type, index, value, placeHolder) {
    ...
    if (type === undefined) {
      $('<span>').html($i18n.t("editor.noDefaultLabel")).appendTo(result);
  ...
  buildLabelForm(container, node) {
    ...
    var inputPlaceholder = node._def.inputLabels ? $i18n.t("editor.defaultLabel") : $i18n.t("editor.noDefaultLabel");
    var outputPlaceholder = node._def.outputLabels ? $i18n.t("editor.defaultLabel") : $i18n.t("editor.noDefaultLabel");
