...
export class CommonUtils extends Context implements ICommonUtils {
  ...
  buildMessageSummaryValue(value: any) {
    ...
    if (Array.isArray(value)) {
      ...
      var subvalue;
      if (value.length > 30) {
        subvalue = this.sanitize(value.substring(0, 30)) + "&hellip;";
      } else {
        subvalue = this.sanitize(value);
      }
      result = $('<span class="debug-message-object-value debug-message-type-string"></span>').html('"' + this.formatString(subvalue) + '"');
    } else {
  ...
  addMessageControls(obj, sourceId, key, msg, rootPath, strippedKey) {
    ...
    if (!!key) {
      var copyPath = $('<button class="editor-button editor-button-small"><i class="fa fa-terminal"></i></button>').appendTo(copyTools).click((e) => {
        e.preventDefault();
        e.stopPropagation();
        this.RED.clipboard.copyText(key, copyPath, "clipboard.copyMessagePath");
      })
    }
    var copyPayload = $('<button class="editor-button editor-button-small"><i class="fa fa-clipboard"></i></button>').appendTo(copyTools).click((e) => {
      e.preventDefault();
      e.stopPropagation();
      this.RED.clipboard.copyText(msg, copyPayload, "clipboard.copyMessageValue");
    })
    if (strippedKey !== '') {
      var isPinned = this.pinnedPaths[sourceId].hasOwnProperty(strippedKey);

      var pinPath = $('<button class="editor-button editor-button-small debug-message-tools-pin"><i class="fa fa-map-pin"></i></button>').appendTo(tools).click((e) => {
        e.preventDefault();
        e.stopPropagation();
        if (this.pinnedPaths[sourceId].hasOwnProperty(strippedKey)) {
          delete this.pinnedPaths[sourceId][strippedKey];
          $(this).removeClass("selected");
          obj.removeClass("debug-message-row-pinned");
        } else {
          var rootedPath = "$" + (strippedKey[0] === '[' ? "" : ".") + strippedKey;
          this.pinnedPaths[sourceId][strippedKey] = this.normalisePropertyExpression(rootedPath);
  ...
  formatNumber(element, obj, sourceId, path, cycle, initialFormat?) {
    ...
    if (cycle) {
      ...
      this.formattedPaths[sourceId] = this.formattedPaths[sourceId] || {};
      this.formattedPaths[sourceId][path] = format;
  ...
  createObjectElement(obj, options) {
    ...
    var element: any = $('<span class="debug-message-element"></span>');
    ...
    if (sourceId) {
      this.addMessageControls(header, sourceId, path, obj, rootPath, strippedKey);
    }
    if (!key) {
      element.addClass("debug-message-top-level");
      if (sourceId) {
        var pinned = this.pinnedPaths[sourceId];
        expandPaths = [];
        if (pinned) {
          for (var pinnedPath in pinned) {
            if (pinned.hasOwnProperty(pinnedPath)) {
              try {
                var res = this.getMessageProperty({
                  $: obj
                }, pinned[pinnedPath]);
                if (res !== undefined) {
    ...
    if (obj === null || obj === undefined) {
      ...
      if (/[\t\n\r]/.test(obj)) {
        ...
        this.makeExpandable(header, function () {
        ...
        }, this.checkExpanded(strippedKey, expandPaths));
      }
      e = $('<span class="debug-message-type-string debug-message-object-header"></span>').html('"' + this.formatString(this.sanitize(obj)) + '"').appendTo(entryObj);
      ...
      if (Number.isInteger(obj) && (obj >= 0)) { // if it's a +ve integer
        ...
        e.click(function (evt) {
          evt.preventDefault();
          this.formatNumber($(this), obj, sourceId, path, true);
        });
      }
      this.formatNumber(e, obj, sourceId, path, false, typeHint === 'hex' ? 'hex' : undefined);
      ...
      if (key) {
        ...
        for (i = 0; i < arrayLength; i++) {
          this.buildMessageSummaryValue(data[i]).appendTo(headerHead);
          if (i < arrayLength - 1) {
            $('<span>, </span>').appendTo(headerHead);
          }
        }
      ...
      if (originalLength > 0) {
        ...
        this.makeExpandable(header, function () {
          ...
          if (type === 'buffer') {
            ...
            var switchFormat = $('<a href="#"></a>').addClass('selected').html('raw').appendTo(bufferOpts).click((e) => {
              e.preventDefault();
              e.stopPropagation();
              this.formatBuffer(element, $(this), sourceId, path, true);
            });
            this.formatBuffer(element, switchFormat, sourceId, path, false);
          ...
          if (fullLength <= 10) {
            for (i = 0; i < fullLength; i++) {
              ...
              subElements[path + "[" + i + "]"] = this.createObjectElement(
                data[i], {
                  key: "" + i,
            ...
            for (i = 0; i < fullLength; i += 10) {
              ...
              this.makeExpandable(header, (() => {
                ...
                this.checkExpanded(strippedKey, expandPaths, minRange, Math.min(fullLength - 1, (minRange + 9))));
      ...
      if (key || keys.length > 0) {
        ...
        this.makeExpandable(header, function () {
          ...
          for (i = 0; i < keys.length; i++) {
            ...
            subElements[newPath] = this.createObjectElement(
              obj[keys[i]], {
                key: keys[i],
                typeHint: false,
                hideKey: false,
      ...
      if (key) {
        ...
        for (i = 0; i < keysLength; i++) {
          ...
          $('<span>: </span>').appendTo(headerHead);
          this.buildMessageSummaryValue(obj[keys[i]]).appendTo(headerHead);
          if (i < keysLength - 1) {
            $('<span>, </span>').appendTo(headerHead);
          }
        }
  ...
  getMessageProperty(msg, expr) {
    ...
    if (typeof expr === 'string') {
      ...
      msgPropParts = this.normalisePropertyExpression(expr);
    } else {
      msgPropParts = expr;
    }
    var m;
  ...
  getNodeLabel(node, defaultLabel) {
    ...
    return this.RED.text.bidi.enforceTextDirectionWithUCC(l);
  }
}
