...
export class NodeEditor extends Context {
  ...
  prepareConfigNodeButton(node, property, type, prefix) {
    ...
    _validateStr(prefix, 'prefix', 'prepareConfigNodeButton')
    _validateStr(property, 'property', 'prepareConfigNodeButton')
    ...
    if (input.length === 0) {
      ...
      logWarning('prepareConfigNodeButton: no such input', {
        selector
      })
      return this;
    }
  ...
  preparePropertyEditor(node, property, prefix, definition) {
    ...
    _validateStr(prefix, 'prefix', 'preparePropertyEditor')
    _validateStr(property, 'property', 'preparePropertyEditor')
    _validateObj(definition, 'definition', 'preparePropertyEditor')

    var selector = "#" + prefix + "-" + property
    var input = $(selector);
    if (input.length === 0) {
      logWarning('preparePropertyEditor: no such element', {
        selector
      })
      return this;
    }
  ...
  attachPropertyChangeHandler(node, definition, property, prefix) {
    ...
    _validateStr(prefix, 'prefix', 'attachPropertyChangeHandler')
    _validateStr(property, 'property', 'attachPropertyChangeHandler')
    _validateObj(definition, 'definition', 'attachPropertyChangeHandler')
    ...
    var input = $(selector);
    if (input.length === 0) {
      logWarning('attachPropertyChangeHandler: no such input', {
        selector
      })
      return this;
    }
  ...
  populateCredentialsInputs(node, credDef, credData, prefix) {
    ...
    _validateStr(prefix, 'prefix', 'populateCredentialsInputs')
  ...
  updateNodeCredentials(node, credDefinition, prefix) {
    ...
    var changed = false;
    _validateStr(prefix, 'prefix', 'updateNodeCredentials')
  ...
  prepareEditDialog(node, definition, prefix, done) {
    ...
    _validateStr(prefix, 'prefix', 'prepareEditDialog')
    _validateNodeDef(definition, 'definition', 'prepareEditDialog')
  ...
  getEditStackTitle() {
    ...
    for (var i = 0; i < editStack.length; i++) {
      ...
      _validateNode(node, 'node', 'getEditStackTitle:iterate')
  ...
  buildEditForm(container, formId, type, ns) {
    ...
    _validateJQ(container, 'container', 'buildEditForm')
  ...
  refreshLabelForm(container, node) {
    ...
    _validateNode(node, 'node', 'refreshLabelForm')
