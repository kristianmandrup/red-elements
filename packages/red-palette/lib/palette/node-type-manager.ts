...
export class PaletteNodeTypeManager extends Context implements IPaletteNodeTypeManager {
    ...
    if (exclusion.indexOf(def.category) === -1) {
      ...
      if ($("#palette-base-category-" + rootCategory).length === 0) {
        if (coreCategories.indexOf(rootCategory) !== -1) {
          createCategoryContainer(rootCategory, $i18n.t("node-red:palette.label." + rootCategory, {
            defaultValue: rootCategory
          }));
        } else {
          var ns = def.set.id;
          createCategoryContainer(rootCategory, $i18n.t(ns + ":palette.label." + rootCategory, {
            defaultValue: rootCategory
          }));
        }
      }
  ...
  setLabel(type, el, label, info) {
    ...
    var currentLineWidth = $view.calculateTextWidth(currentLine, "palette_label", 0);
    ...
    try {
      ...
      popOverContent = $(l + (info ? info : $("script[data-help-name='" + type + "']").html() || "<p>" + $i18n.t("palette.noInfo") + "</p>").trim())
        .filter(function (n) {
      ...
      popOverContent = "<p><b>" + label + "</b></p><p>" + $i18n.t("palette.noInfo") + "</p>";
    }
